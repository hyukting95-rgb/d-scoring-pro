...
import { PersonnelRecord, ProjectRecord, User } from '../types';
...
const PersonnelLeaderboard: React.FC<PersonnelLeaderboardProps> = ({ currentUser, records, projectRecords }) => {
  ...
  const staffSummary = React.useMemo(() => {
    ...
    // 权限过滤：管理员看全员，普通用户只看自己
    const visibleRecords = records.filter(r => isAdmin || r.createdBy === currentUser.id);
  ...
  }, [records, projectRecords, isAdmin, currentUser.id]);
  ...
  if (staffSummary.length === 0) {
    return (
      <div className="p-20 text-center tech-card rounded-[2rem] border-dashed border-2 border-slate-800">
        ...
        <p className="text-slate-500">暂无符合权限要求的已完成积分数据</p>
      </div>
    );
  }
    ...
    <div className="tech-card rounded-[2rem] p-8 space-y-8">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-white flex items-center gap-2">
          ...
          {isAdmin ? '全员积分贡献榜单 (管理员视图)' : '个人积分表现概览'}
        </h3>
        <span className="text-xs text-slate-500 font-mono">
           {isAdmin ? `共 ${staffSummary.length} 位活跃成员` : '数据仅包含本人录入'}
        </span>
      </div>
